...
The `@bcgov/catalog-openapi` package provides the frontend UI for rendering `OpenAPI` entities in Backstage.
...
```tsx
<EntitySwitch.Case if={isKind('openapi')} children={<CatalogRestApiPage />} />
```

In normal usage, users access this page by opening a `Rest Api` entity from the Backstage catalog.

A direct route such as `/catalog-openapi` does **not** provide entity context and will fail unless additional context wiring is added.

---

## Local Development

This plugin can still be developed locally, but the standard Backstage usage is through a openapi entity in the catalog.
...
- Do not document `/catalog-openapi` as the normal access path unless you intentionally add entity context for that route